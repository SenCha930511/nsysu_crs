...
| **學生專區** | 在學學生（SSO2 認證） | 校內「我的已選」即時同步、暫存清單加退選代送（支援志願序）、送出前表單校對預覽（Preview）與**密碼二次確認防呆機制**、非同步背景工作佇列（Redis Queue + Background Worker）、即時送單紀錄與執行結果追蹤（精確顯示校內反饋如「違反限修條件」，學號自動遮罩脫敏）。**個人專區（我的）**：歷年成績一鍵同步（會話範圍快照、登出即刪）、學雜費繳費狀態即時查詢（不落地）、在學證明 PDF 直接下載（伺服器記憶體串流轉交、不快取不落盤）——登入時在背景一次性建立校內各子系統連線，任一失敗皆不影響登入。 |
