# 中山學生 Studio（NSYSU Student Studio）
...
> [!TIP]
> 🌐 **線上正式版已上線**：[https://nsysucrs.senchaaa.com](https://nsysucrs.senchaaa.com)

專為國立中山大學（NSYSU）設計的一站式學生助：流暢課程檢索與多維度進階篩選、即時週課表排課與衝堂／學分檢查、站內課程大綱浮層（名稱點選延伸至學校官方大綱頁）、課表 PNG 圖檔與 **iCalendar (.ics) 行事曆匯出**、校內已選課程同步；登入後再加值：歷年成績一鍵同步與異動 diff、學雜費繳費狀態即時查詢與 **收據 PDF 下載**、在學證明 PDF 直下、**本學期註冊事項 checklist**。在嚴格的**二次確認機制**與高規格資訊安全保障下，支援非同步代理加退選功能。
...
| **排課模擬** | 訪客／免登入 | 本地暫存排課（localStorage）、視覺化週課表預覽、即時衝堂警示、學分與節數自動統計、PNG 高解析度課表圖檔匯出、**iCalendar (.ics) 行事曆匯出**（可直接匯進 Apple／Google 行事曆囕教育部期節奏）。 |
| **學生專區** | 在學學生（SSO2 認證） | 校內「我的已選」即時同步、暫存清單加退選代送（支援志願序）、送出前表單校對預覽（Preview）與**密碼二次確認防呆機制**、非同步背景工作佇列（Redis Queue + Background Worker）、即時送單紀錄與執行結果追蹤（精確顯示校內反饋如「違反限修條件」，學號自動遮罩脫敏）。**個人專區（我的）**：歷年成績一鍵同步（會話範圍快照、登出即刪；同步回報「新增／移除／不變」異動摘要）、學雜費繳費狀態即時查詢（不落地）與 **收據 PDF 直下**、在學證明 PDF 直接下載（伺服器記憶體串流轉交、不快取不落盤）、**本學期註冊事項 checklist**（未完成事項提醒）——登入時在背景一次性建立校內各子系統連線，任一失敗皆不影響登入。 |
