...
- **在學證明 electronic cert**: CONFIRMED REACHABLE (supersedes the UNVERIFIED verdict above) - the regweb checklist carries a 「產生在學證明 / Certificate of Enrollment」 **BUTTON** (``onclick window.open``, invisible to anchor-only discovery - the reason earlier rows misjudged): ``WRegMain3.asp?act=71&out=print/enrollcert.asp`` -> WregRedirect 302 -> auto-form handoff POST -> ``print/enrollcert.asp`` answers **application/pdf** (2026-09-04 live: valid %PDF-1.4, 1,865,701 bytes, md5 7cfe365353e7515023b734063b4da774; not committed for size - passthrough needs no parser; credentials masked in the artifact during the round).
- **session TTL (all subsystems)**: UNVERIFIED - t+0 alive everywhere; sliding/hard bounds not probed by design (M0 scope).

### capture run 2026-09-06 12:01 (Asia/Taipei) - window stu_enroll M0
