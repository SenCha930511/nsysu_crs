# Operations Runbook — NSYSU Student Studio (中山學生 Studio)
...
| `FEATURE_STU_ENROLL` | `false` | stu_enroll M2/M3 switch: gates the login X-fan-out to the regweb/sco campus families and every `/api/me/*` endpoint (payment-status, payment-receipt, enrollment-cert, grades+sync, registration-checklist). OFF = endpoints answer 404 `not_found` and logins skip the fan-out entirely (zero added login latency). |
