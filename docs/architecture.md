...
- **stu_enroll family (M2; stu-enroll-plan §5.4 decision)** — `/api/me/payment-status` and `/api/me/enrollment-cert` wire the same way (local jar check, then `admit()`; relay bounce is coherent, drift feeds the streak). The login-time two-family fan-out deliberately never calls `admit()` (fail-soft must not block site login) and records **school verdicts only**: an available leg `record_classified`, a `SCHOOL_UNAVAILABLE` leg `record_unknown`, and our-side failures (captcha budget, fan-out deadline, app bug) record nothing — so the fan-out can neither clear nor accelerate a genuine failure streak.
- **Fan-out is asynchronous since 2026-09-06 (login-latency fix, user-reported).** The login round-trip no longer awaits the two-family fan-out: after SSO2 SUCCESS + site session + selcrs park, the subsystem logins are scheduled as an `asyncio.create_task` (`_run_fanout_and_store`, keepalive set, follow-through identical: jars park with the sliding+hard pair, breaker school-verdicts only, wrapped so any failure stays log-only). The response reports `regweb_available=false / sco_available=false / stu_enroll_pending=true`; the family's availability is derived from jar presence, so the frontend polls `/api/auth/me` (2.5s, max 6) and the flags flip once the task lands — verified locally 0.2s login with pills flipping within ~8s, and on prod 1.2–1.5s with `/api/auth/me` true at ~7s (was: 3–21s blocked, OCR-luck-dependent). Hidden-trap fixed alongside: `Settings` also reads the repo-root `.env`, and a local flag-on silently leaked into tests (unstubbed logins spawned REAL background school calls) — `tests/conftest.py` now pins both feature flags to `false` session-wide (`monkeypatch.setenv`), explicit-kwarg tests still override.
