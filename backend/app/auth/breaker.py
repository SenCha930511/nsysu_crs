...
_STREAK_KEY: Final = "breaker:school:streak"
_OPENED_AT_KEY: Final = "breaker:school:opened_at"
_PROBE_KEY: Final = "breaker:school:probe"
_PROBE_GATE_SECONDS: Final = 60
...
class SchoolBreaker:
    ...
    async def admit(self) -> bool:
        ...
        """
        opened_at = await self._redis.get(_OPENED_AT_KEY)
        ...
        granted = await self._redis.set(
            _PROBE_KEY, uuid.uuid4().hex, nx=True, ex=_PROBE_GATE_SECONDS
        )
        return bool(granted)

    async def record_classified(self) -> None:
        """School answered coherently (SUCCESS or CREDENTIAL-FAIL): reset all."""
        await self._redis.delete(_STREAK_KEY, _OPENED_AT_KEY, _PROBE_KEY)

    async def record_unknown(self) -> int:
        """Feed one UNKNOWN/transport failure; open at threshold. Returns the streak."""
        streak = await self._redis.incr(_STREAK_KEY)
        await self._redis.expire(_STREAK_KEY, 2 * self.recovery_after)
        if streak >= self.failure_threshold:
            ...
            await self._redis.set(_OPENED_AT_KEY, repr(self._now()))
            await self._redis.delete(_PROBE_KEY)
        return streak
